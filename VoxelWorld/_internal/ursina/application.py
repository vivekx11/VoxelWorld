...
# --- PyInstaller EXE fix ---
# When frozen, sys._MEIPASS is the extracted bundle folder.
# asset_folder should still be next to the EXE (not inside _MEIPASS).
if getattr(sys, 'frozen', False):
    asset_folder = Path(sys.executable).parent
else:
    asset_folder = Path(sys.argv[0]).parent

blender_paths = dict()

# Force development_mode OFF so Ursina never tries to call Blender
