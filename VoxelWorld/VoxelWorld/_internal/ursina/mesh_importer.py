...
def load_model(name, path=Func(getattr, application, 'asset_folder'), file_types=('.bam', '.ursinamesh', '.obj', '.glb', '.gltf'), use_deepcopy=False, gltf_no_srgb=Func(getattr, application, 'gltf_no_srgb')):
    if callable(path):
        path = path()
    ...
    for filetype in file_types:
        ...
        for filename in path.glob(f'**/{name}{filetype}'):
            ...
            if filetype == '.obj':
                ...
                # .blend files not supported in packaged EXE (no Blender installed)
                # use .ursinamesh files from models_compressed instead
                pass
            else:
                try:
                    return builtins.loader.loadModel(filename)  # type: ignore
